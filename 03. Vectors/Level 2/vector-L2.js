...
Vector.prototype.insert = function(index, value) {
   //resize. double only since adding to this.storage
   if (this.length === this.capacity && this.capcity <= this.max) {
      this.resize("double");
   }
   //Why not create a temporary array? I was storing it in a new array, but my length was always too much. What caused this error?

   //shift all values after index of of inserted value
   for (var i = this.length; i >= index; i--) {
      this.storage[i] = this.storage[i-1];
   }

   //assign desired value to indicated index
...
Vector.prototype.add = function(value) {
   //resize. double only- because if adding, no need to reduce.
...
Vector.prototype.remove = function(index) {
   //resize. half only because no need to double if removing.
...
Vector.prototype.resize = function(action) {
   if(action === "double"){
      this.capacity *= 2;
